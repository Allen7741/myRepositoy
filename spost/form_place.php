...
if (isset($_REQUEST["title"])) {
    ...
    //資料庫
    $sql = "INSERT INTO placetest (title, addr, info, plimit, price, pricepertime, mon, tue, wed, thu, fri, sat, sun, type) VALUES " 
    . "('{$title}','{$addrs}','{$info}','{$plimit}','{$price}','{$pricepertime}','{$mon}','{$tue}','{$wed}','{$thu}','{$fri}','{$sat}','{$sun}','{$stype}')";
    if ($mysqli->query($sql)) {
        echo "上傳成功";
    } else {
        echo "上傳失敗";
    ...
    // 資料庫
    if ($img3 !== null ){
        $sqlimg = "INSERT INTO pimgtest (img) VALUES('{$img1}'),('{$img2}'),('{$img3}')";
    }
    elseif($img2 !== null){
        $sqlimg = "INSERT INTO pimgtest (img) VALUES('{$img1}'),('{$img2}')";
    }
    else{
        $sqlimg = "INSERT INTO pimgtest (img) VALUES('{$img1}')";
    }
